...
> for (x in file[num:num]){
	mdf<-lmer(fitness~x+(1|block))
	md<-lmer(fitness~x+I(x^2)+(1|block))
	print(summary(mdf))
	print(summary(md))

# In case phenotypic measurements are missing, it's necessary to use "na.omit" function
> mdf<-lmer(fitness~x~(1|block), na.action=na.omit)
